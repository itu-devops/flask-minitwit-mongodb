...
```bash
$ # Due to some operating systems, i.e. macOS, using port 5000, we instead use port 5001 outside Docker.
$ docker run -d -p 27017:27017 --name dbserver your_id/dbserver
$ docker run -it -d --rm --name webserver --link dbserver -p 5001:5000 your_id/webserver
```
...
97cd4f08c246        your_id/webserver   "python ./minitwit.py"   About a minute ago   Up About a minute   0.0.0.0:5001->5000/tcp     webserver
27cd3df694c4        your_id/dbserver    "docker-entrypoint.s…"   7 minutes ago        Up 7 minutes        0.0.0.0:27017->27017/tcp   dbserver
```

Now, point your browser to http://localhost:5001 and see that the application is running.
...
$ docker run -it -d --rm --name webserver --network=minitwit-network -p 5001:5000 your_id/webserver
```

Again, point your browser to http://localhost:5001 and see that the application is running on the newly created network.
...
services:
  ...
  webserver:
    ...
    ports:
      - "5001:5000" # Depending on the OS using port 5000 might be reserved. If not, change to "5000:5000" or something similar.
    healthcheck:
